...
try:
    ...
    print("Failed to import TRT. Only torch is supported!")
    pass
...
def initializeTorch(processNum, outfile):
    ...
    print("D2: Using cuda, ", use_cuda, flush=True, file=outfile)
    
    if processNum < torch.cuda.device_count():
        devname = "cuda:" + str(processNum)
        print("initialize device ", devname, file=outfile)
        device = torch.device(devname)
        print(device, file=outfile)
        model = D2Net(
...
def initializeTRT(processNum, fp16, outfile):
    ...
    if processNum < torch.cuda.device_count():
        ...
        model.to(device)
        print("D2: Initialization complete", flush=True, file=outfile)
        return device, model
...
def processImage(image, localDevice, localModel, method):
    ...
    #print(localDevice, " processing image took ", elapsed, "seconds", flush=True, file=outfile)
    print(localDevice, " processing image took ", elapsed, "seconds", flush=True)
...
class D2Parameters():
    def __init__(self, rawImage, maxNrFeatures = 2000, rotation = None, intrinsics = None, maxSideLength = None, targetFocalLength = None, integerFeature = False):
...
class D2Procedure():
    def __init__(self, method="torch", fp16="off", outfile=None):
        ...
        if method == "trt" and trtavailable == False:
            method = "torch"
            print("ERROR: trt was specified, but could not be imported! Make sure tensorRT is correctly installed! Falling back to torch", flush=True, file=self.outfile)
